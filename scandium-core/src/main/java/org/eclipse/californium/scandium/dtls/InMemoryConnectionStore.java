...
public class InMemoryConnectionStore implements ResumptionSupportingConnectionStore {
	...
	protected static final Logger LOG = LoggerFactory.getLogger(InMemoryConnectionStore.class.getName());
	...
	public synchronized boolean put(final Connection connection) {
		...
		if (connection != null) {
			...
			if (connections.put(connectionId, connection)) {
				...
				if (session != null) {
					session.setPeer(connection.getPeerAddress());
	...
	@Override
	public synchronized void removeEstablishedSession(final DTLSSession session, final Connection connection) {
		SessionId sessionId = session.getSessionIdentifier();
		if (!sessionId.isEmpty()) {
