...
public class DTLSConnector implements Connector, RecordLayer {
	 ...
	 */
	private final Connection getConnection(InetSocketAddress peerAddress, boolean create) {
		...
		synchronized (connectionStore) {
			...
			if (connection == null) {
				...
				LOGGER.debug("revive connection for {}", connection);
				connection.setExecutor(new SerialExecutor(executor));
			} else {
				LOGGER.trace("connection available for {}", connection);
			}
			return connection;
		}
	}
	...
	public final void send(final RawData msg) {
		...
		if (!running.get()) {
			...
			connection = getConnection(msg.getInetSocketAddress(), !serverOnly);
			if (connection == null) {
				if (serverOnly) {
		...
		if (session == null && ticket == null) {
			...
			if (connection.isAutoResumptionRequired(timeout)) {
				...
				if (session != null) {
					...
					connectionStore.removeEstablishedSession(session, connection);
				} else {
